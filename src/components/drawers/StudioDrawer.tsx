...
function ActiveProject({ state, advance, doUpdateProject, doAddTrack, doRemoveTrack, doConfigureTrackStage, doFinishProject, doScrubProject, doTakeStudioBreak, doPushThrough, doCancelStudioChoice, doAutoGenerateTracks, onBack, onClose }: any) {
  ...
  return <div className="drawer-overlay" onClick={onClose}><div className="drawer studio-drawer song-studio-drawer" onClick={event => event.stopPropagation()}>
    ...
    <section className="studio-mini-panel" aria-label="Project direction">
      <div className="studio-option-label">Project direction</div>
      <div className="studio-option-buttons">{(Object.keys(PROJECT_DIRECTIONS) as ProjectDirection[]).map(direction => <button key={direction} className={`btn btn-sm ${(project.creativeDirection ?? "balanced") === direction ? "btn-lime" : ""}`} aria-pressed={(project.creativeDirection ?? "balanced") === direction} disabled={!canChangeProjectDirection(project)} onClick={() => doUpdateProject({ creativeDirection: direction })}>{PROJECT_DIRECTIONS[direction].label}</button>)}</div>
      <p className="tip-text">{PROJECT_DIRECTIONS[(project.creativeDirection ?? "balanced") as ProjectDirection]?.description} Direction locks after your first writing pass.</p>
    </section>
...
function TrackDevelopmentCard({ index, track, stage, state, studioTier, doConfigureTrackStage, doRemoveTrack }: any) {
  ...
  return <article className="song-development-card"><div className="song-development-heading"><div><span className="t-num">{index + 1}</span><strong>{track.name}</strong></div>{stage === "writing" && <button className="btn btn-sm btn-danger" onClick={() => doRemoveTrack(index)}>Remove</button>}{stage === "complete" && quality !== null && <div className="song-score-pair"><span>Q {quality.toFixed(1)}</span><span>A {(appeal ?? 5).toFixed(1)}</span></div>}</div>
    ...
    <p className="tip-text">{development.strengths ? "Song strengths" : "Creative profile preview"} · Lyrics {strengths.lyrics.toFixed(1)} · Hook {strengths.hook.toFixed(1)} · Live {strengths.live.toFixed(1)} / 10</p>
