...
export default function SetupScreen(game: any) {
  ...
  const handleStart = () => {
    if (typeof game?.startNewGame !== "function") {
      console.error("startNewGame is not a function. Check parent props.", game);
      return;
    }
  ...
  return (
    <div style={{ padding: "24px 16px 40px" }}>
      <div className="pg-title" style={{ marginBottom: 16 }}>
        New Career
      </div>
      ...
      <div className="field">
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <input
            ...
            placeholder="Enter name or roll the dice"
            style={{ flex: 1 }}
          />
          <button
            type="button"
      ...
      <div className="field">
        ...
        <div className="g2">
          {(["Country", "Blues"] as Genre[]).map((g) => (
            <button
              key={g}
              className={`btn ${genre === g ? "btn-lime" : ""}`}
              onClick={() => setGenre(g)}
            >
              {g}
            </button>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {Object.values(ARCHETYPES)
            ...
            .map((a: any) => (
              <div
                key={a.id}
                className={`pick-card ${arch === a.id ? "sel" : ""}`}
